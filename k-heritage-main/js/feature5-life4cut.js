class Life4CutFeature {
    ...
    async init() {
        const patternPaths = ['assets/dancheong/pattern1.png', 'assets/dancheong/pattern2.png'];
    ...
    renderUI() {
        ...
        container.innerHTML += `
            ...
            <div class="frame-indicator">촬영된 컷: <span id="frame-count">0</span>/4</div>
        `;
    ...
    captureFrame() {
        if (this.capturedFrames.length >= 4) { alert('4 컷이 모두 촬영되었습니다!'); return; }
        const video = document.getElementById('webcam');
    ...
    renderAllFrames() {
        ...
        const frameWidth = 380, frameHeight = 280, gap = 20;
        this.capturedFrames.forEach((frame, index) => {
            const row = Math.floor(index / 2), col = index % 2;
            const x = col * (frameWidth + gap), y = row * (frameHeight + gap);
    ...
    downloadResult() {
        if (this.capturedFrames.length < 4) { alert('4 컷을 모두 촬영해주세요!'); return; }
